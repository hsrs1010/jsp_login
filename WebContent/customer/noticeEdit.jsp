	...
	<form action="noticeEditProc.jsp" method="post">
		<table class="twidth">
			...
			<tbody>
				...
				<tr>
					<th class="left">제목</th>
					<td colspan="3"><input class="inp" name="title"
						value="<%=rs.getString("title")%>" /></td>
				</tr>
				<tr>
					<th class="left">내용</th>
					<td colspan="3" id="content"><textarea class="txt"
							name="content"><%=rs.getString("content")%></textarea></td>
				</tr>
				<tr>
					<th class="left">첨부</th>
		...
		<div>
			<input type="hidden" name="c" value="<%=rs.getString("seq")%>" /> <input
				type="submit" value="저장" /> <a
				href="noticeDetail.jsp?c=<%=rs.getString("seq")%>">취소</a>
		</div>
	</form>
